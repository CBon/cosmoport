        ...
        if (checkValid(id)) {
            ...
            if (shipService.isExist(idLong)) {
                ...
                if (ship.getName() == null && ship.getPlanet() == null && ship.getShipType() == null && ship.getProdDate() == null &&
                    ...
                    if (ship.getName() == null) ship.setName(shipService.getShip(idLong).getName());
                    else ship.setName(ship.getName());
                    if (ship.getName().isEmpty()) throw new BadRequestException();
                    if (ship.getPlanet() == null) ship.setPlanet(shipService.getShip(idLong).getPlanet());
                    else ship.setPlanet(ship.getPlanet());
                    if (ship.getShipType() == null) ship.setShipType(shipService.getShip(idLong).getShipType());
                    else ship.setShipType(ship.getShipType());
                    if (ship.getProdDate() == null) ship.setProdDate(shipService.getShip(idLong).getProdDate());
                    else ship.setProdDate(ship.getProdDate());
                    calendar.setTime(ship.getProdDate());
                    if (calendar.get(Calendar.YEAR) < 2800 ||
                            calendar.get(Calendar.YEAR) > 3019) throw new BadRequestException();
                    if (ship.getUsed() == null) ship.setUsed(shipService.getShip(idLong).getUsed());
                    else ship.setUsed(ship.getUsed());
                    if (ship.getSpeed() == null) ship.setSpeed(shipService.getShip(idLong).getSpeed());
                    else ship.setSpeed(ship.getSpeed());
                    if (ship.getCrewSize() == null) ship.setCrewSize(shipService.getShip(idLong).getCrewSize());
                    else ship.setCrewSize(ship.getCrewSize());
                    if (ship.getCrewSize() < 1 || ship.getCrewSize() > 9999) throw new BadRequestException();
                    calendar.setTime(ship.getProdDate());
                    Double rating = Math.rint(100.0 * ((80 * ship.getSpeed() * (ship.getUsed() ? 0.5 : 1)) / (3019 - calendar.get(Calendar.YEAR) + 1))) / 100.0;
